...
/*   Updated: 2019/02/01 14:09:50 by apeyret          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	inside_dquote_qr(t_str *str_w, int *index)
{
	...
	while (str_w->str[*index])
	{
		...
		{
			shift_str_left(str_w, *index);
			if (str_w->str[*index]) /* we should reject backslahs at endofline in lexer */
				*index = *index + 1;
		}
		else
			*index = *index + 1;
...
int	quote_removal(t_token *word)
{
	...
	while (word->data.str[index])
	{
		...
		else if (word->data.str[index] == '\\')
		{
			...
			if (word->data.str[index]) /* we should reject backslahs at endofline in lexer */
				index = index + 1;
		}
		else
			index++;
